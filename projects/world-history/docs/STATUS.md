...
Phase 5 synchronizes selection across every panel: the box list, alluvial list, Sankey diagram, and timeline all stay consistent no matter where a box or link is selected. The Sankey diagram and timeline don't have a `selectedLinkId`/`selectedBoxId` field of their own — a selected box gets an outline (Sankey node border, timeline bar stroke), and a selected link's two endpoint boxes get an outline (Sankey `link.color`, timeline `.link-endpoint`), all applied via `js/app.js` re-deriving the highlight on every store change, not by the Sankey/timeline modules knowing about each other.

Phase 5.5 (`docs/timescaleRequirement.md`) replaces the timeline's plain linear x-axis with a shared, pure, Node-testable "semantic zoom" scale (`js/timescale.js`'s `semanticTimeScale`/`timeTicks`): `x = 1 - (a/M)^p`, where `p` is driven by both the visible span (`state.yearStart`/`yearEnd`) and a new "Time compression" slider (`state.timeScale`, range [-2, 2], default 0). The same scale instance/domain now positions timeline bars, SEAI markers, axis ticks, *and* the Sankey diagram's node x-position (`js/sankey.js`'s `computeSankeyNodeX`) — dragging the slider or the year-range inputs updates both views together, continuously (bound to the slider's `input` event, not `change`). The Sankey diagram's node *set* is still driven only by `data/links.json`, unfiltered by year range — only where each node lands on the x-axis changed. Axis labels are now explicit "N BCE"/"N CE" instead of a raw signed number. Smoothly animating the transition while dragging (optional per the spec) was not implemented — see `ISSUES.md`.
